...
public class TicTacToeGame
{
	...
	private boolean XtoMove = true;        // true if it is X's turn, false if it is O's turn
	...
	public TicTacToeGame copy() {
		...
		game.Xscore = this.Xscore;
		game.Oscore = this.Oscore;
		game.XtoMove= this.XtoMove;
		for(int x=0;x<3;x++){
			for(int y=0;y<3;y++){
	...
	public void playMove(TicTacToeMove move) {
		if (move != null && canPlacePieceAt(move.moveX, move.moveY)){
			placePieceAt (move.moveX, move.moveY);
			switchCurrentPlayer();
		}
	}
	/**
	...
	public void playMove(Point move) {
